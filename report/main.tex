...
\begin{algorithm} \label{solveAlgo}
	\caption{Automated First Order Natural Deduction Proof Solver (adapted from \cite{ndAlgo})}\label{euclid}
		...
		In this algorithm I represent a proof as $P \vdash G$, where the set \textit{P} are the premises and \textit{G} is the goal of a proof. The two lists used in the algorithm are \textit{list\_proof}, to store the current state of the proof and \textit{list\_goal}, to store the current goals and sub-goals of the proof. $G_{n}$ will also be used to keep track of the current goal.
		
		Reachability of the current goal shows whether an expression is in scope and can therefore be accessed at a stage in the proof.
